...
class ViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
        ...
        if use_abs_pos_emb:
            if self.use_cls_token:
                ...
                self.cls_token_pos_embed = nn.Parameter(torch.zeros(1, 1, embed_dim), requires_grad=learnable_pos)
    ...
    def forward_features(self, x, mask=None):
        ...
        if self.use_cls_token:
            ...
            pos_embed = torch.cat([self.cls_token_pos_embed, self.pos_embed], dim=1)
        ...
        if self.test_pos_mode is False:
            if x.size(1) == pos_embed.size(1):
                x = x + pos_embed  # BxHWxC
            else: # take top-left if pos_embed > x's dimension
                x = x + pos_embed.reshape(1, self.patch_embed.patch_shape[0],
                                            self.patch_embed.patch_shape[1],
                                            pos_embed.size(2))[:,:Hp, :Wp, :].reshape(1, x.size(1), pos_embed.size(2))
            ...
            pos_embed = pos_embed.reshape(-1, orig_size[0], orig_size[1], pos_embed.shape[-1]).permute(0, 3, 1, 2)
            ...
            pos_embed = get_2d_sincos_pos_embed(pos_embed.shape[-1], (Hp, Wp), cls_token=False)
            ...
            # as in original scale
            pos_embed = get_2d_sincos_pos_embed(pos_embed.shape[-1], orig_size, cls_token=False)
            ...
            patch_shape = (Hp, Wp)
            x = x + get_abs_pos(pos_embed, self.use_cls_token, patch_shape)
        else:
...
def vit_base_patch16_ladder_attention_share_pos_embed(pretrained=False, load_pos_embed=True, **kwargs):
    default = dict(
    ...
    if pretrained:
        ...
        if pretrained == 'supervised-80ecf9dd':
            ...
            rel_path = "asset/weights/PATH-ViT-B.pth"
...
def vit_large_patch16_ladder_attention_share_pos_embed(pretrained=False, load_pos_embed=True, **kwargs):
    default = dict(
...
def vit_base_patch16_ladder_attention_share_pos_embed_ema(**kwargs):
    backbone = vit_base_patch16_ladder_attention_share_pos_embed(**kwargs)
    backbone.ema = [vit_base_patch16_ladder_attention_share_pos_embed(**kwargs)]
...
def get_abs_pos(abs_pos, has_cls_token, hw):
    """
    Resize absolute positional embeddings to match (h, w), handling cls_token if present.
    Args:
        abs_pos (Tensor): (1, num_position, C).
        has_cls_token (bool): whether abs_pos includes a cls token.
        hw (Tuple): target (h, w).
    Returns:
        (1, h * w, C) or (1, 1 + h * w, C)
    """
    h, w = hw
    if has_cls_token:
        cls_pos = abs_pos[:, :1]  # (1, 1, C)
        abs_pos = abs_pos[:, 1:]  # remove cls token

    xy_num = abs_pos.shape[1]
    C = abs_pos.shape[2]
    
    abs_pos = abs_pos.reshape(1, xy_num, 1, C).permute(0, 3, 1, 2)
    ...
    abs_pos = abs_pos.permute(0, 2, 3, 1).reshape(1, h * w, C)

    if has_cls_token:
        abs_pos = torch.cat([cls_pos, abs_pos], dim=1)
